...
- La barre de progression ne coûte aucune réplication : elle est armée par l'acquittement `InteractionStarted`, qui porte déjà l'échéance décidée par l'executor (`TryGetExecutionProgress(out actionId, out progress)` ; `TryGetGestureProgress` fait la même chose pour le seuil, lui purement local). Sur un client distant, rien n'est dessiné pendant un aller-retour — c'est le prix d'une seule source pour la durée, voir plus bas. Le serveur garde le chrono autoritaire, et une prédiction qui dérive est corrigée par l'état monde qu'elle attend.
...
- **La durée n'entre dans le système qu'à un seul endroit : ce que l'executor retourne.** `ExpectedDuration` est supprimée. Elle exposait la même donnée deux fois — une propriété déclarée à côté du code qui décide —, et les deux pouvaient se contredire : un `RunningFor` qui ne dit pas la même chose que l'Inspector, ou une durée calculée depuis un état que le serveur seul possède. Le client lisait la déclaration, l'autorité appliquait le retour, et [l'acquittement portait déjà la bonne valeur sans jamais reconcilier la prédiction](../../../addons/interaction_plugin/runtime/interactor/InteractionInteractor.cs) : la barre était donc silencieusement fausse dans le cas même pour lequel la surcharge existait. Il reste `RunningFor(seconds)` et `RunningUntilCompleted()`, `protected static` sur `InteractionActionExecutor` parce que c'est là qu'on écrit `return`, et un seul sens pour le nombre : positif, c'est une échéance ; sinon il n'y en a pas.
- Un executor dont la longueur doit être authorée l'expose lui-même — `[Export] Duration` sur `TransitionStateInteractionExecutor`, passée à `RunningFor(Duration)`. La donnée reste dans la scène, mais elle traverse le code qui l'utilise, donc l'Inspector et le chrono ne peuvent plus diverger.
- **Le coût assumé : la barre d'un client distant démarre sur l'acquittement, un RTT après la pression.** Aucun peer ne peut prédire une valeur qui vit dans du code exécuté ailleurs, et une barre juste en retard d'un ping vaut mieux qu'une barre immédiate que l'autorité contredit. Offline et listen host ne voient aucune différence, l'acquittement y étant synchrone. Prouvé par `ARemoteClientDrawsNoBarUntilTheAuthorityAnswers`. Si le délai gêne un jour, la réponse est un état « démarrage » dans le widget, pas une seconde donnée dans l'API.
- Deux conséquences du fait que la barre appartient désormais à une exécution acquittée et non à la dernière requête. Un refus ne l'efface plus : rappuyer sur l'action qu'on est déjà en train de faire renvoie « This is already in use. », et la barre du hack en cours doit survivre (`ARejectionLeavesTheBarOfARunningExecutionAlone`). Et un acquittement terminal l'efface, même sans relâchement d'input : une exécution annulée avant son échéance emporte sa barre au lieu de la laisser descendre vers une fin qui n'existe plus (`AnExecutionEndedBeforeItsDeadlineTakesItsBarWithIt`).
