...
- La barre de progression ne coûte aucune réplication : elle est calculée localement par la query de durée de l'executor, puis recalée par l'acquittement `InteractionStarted` (`TryGetExecutionProgress(out actionId, out progress)` ; `TryGetGestureProgress` fait la même chose pour le seuil, lui purement local). Rien n'est répliqué et rien n'attend : le client dessine à la pression, l'autorité corrige à l'acquittement. Le serveur garde le chrono autoritaire, et une prédiction qui dérive est corrigée par l'état monde qu'elle attend.
...
- Les capacités restent disponibles même au repos : `IsHoldable` dérive de `HoldThreshold > 0`, et
  `HasTimedExecution` d'une durée positive répondue par `ComputeInteractionDuration(context)`.
  `HoldProgress` et `ExecutionProgress` restent nullables : les booléens disent qu'une barre a du sens,
  les progressions disent qu'elle est actuellement en train d'avancer.
...
- Le prompt d'action par défaut affiche sa barre de hold dès que `IsHoldable` est vrai, à zéro tant que
  le geste n'a pas commencé. Il n'affiche pas la progression d'exécution : `HasTimedExecution` et
  `ExecutionProgress` sont disponibles pour un widget d'objet ou une présentation de jeu dédiée.
