...
3. Ajouter `InteractionArea` et `InteractiveComponent` au propriétaire Node3D, puis assigner `InteractionArea` et `InteractionOwner` dans l'inspecteur. Ajouter et assigner un `InteractionStateful` seulement si l'objet a besoin d'un état persistant/répliqué. `IndicationArea` et `InteractionAnchor` restent facultatifs. Configurer `BusyReason` et `ActivatedReason` lorsque les blocages internes nécessitent un texte métier, par exemple `Talking...` pendant un dialogue.
