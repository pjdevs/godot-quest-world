...
1. ~~`InteractionExecutionRunning(0)` signifie « reprendre `ExpectedDuration` », tandis que `ExpectedDuration == 0` signifie « aucune deadline ».~~ Corrigé bien plus profondément que par des factories. Nommer les cas sans supprimer la double exposition laissait un nom qui mentait, et surtout laissait la vraie faute : une propriété authorée à côté du code qui décide permet d’author 2 s et de retourner 5, donc la prédiction client ne valait rien. `ExpectedDuration` est **supprimée** et remplacée par une query pure et publique, `ComputeInteractionDuration(InteractionContext)`, défaut `0`, que l’autorité et le client propriétaire exécutent tous les deux : le premier via `RunningForDuration(context)`, le second pour dessiner sa barre à la pression. Une seule source, deux lecteurs.

   `RunningFor(seconds)` n’existe pas, délibérément : elle rouvrirait la divergence. Le seul moyen d’obtenir une échéance est donc une fonction que tous les peers peuvent appeler ; une longueur connue seulement après le démarrage passe par `RunningUntilCompleted()` plus une complétion gameplay, et n’était de toute façon prédictible par personne. Lire un état que le client n’a pas reste possible : c’est une incohérence choisie, fausse pendant un aller-retour, notée pour l’implémenteur et non interdite par le core — le même contrat qu’une `InteractionRule`.

   L’autorité garde le dernier mot sans rien répliquer : la durée voyage une fois, dans l’acquittement `InteractionStarted`, au seul demandeur, qui arme, retime ou efface la barre prédite — et décale son échéance de l’aller-retour qu’il vient de mesurer, sans quoi une prédiction juste finirait un RTT trop tôt et laisserait un trou entre la barre pleine et l’effet. La prédiction porte l’`ExecutionId` acquitté — `0` tant qu’elle n’est qu’une prédiction — et c’est cette distinction qui permet à un refus d’effacer la barre inventée par le client qui perd une course sans jamais toucher celle d’une exécution acquittée, quand le joueur rappuie sur l’action qu’il est déjà en train de faire. Sept tests : trois branches de recalage, la compensation de latence, la course perdue, le refus sur une exécution en cours, l’annulation avant l’échéance.
